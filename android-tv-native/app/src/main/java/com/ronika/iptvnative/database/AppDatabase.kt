...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        // Migration from version 12 to 13: Add cloud sync fields to user table
        private val MIGRATION_12_13 = object : Migration(12, 13) {
            override fun migrate(database: SupportSQLiteDatabase) {
                // Check existing columns
                val cursor = database.query("PRAGMA table_info(user)")
                var hasCloudUserId = false
                var hasCloudEnabled = false
                var hasSubscriptionEnabled = false
                while (cursor.moveToNext()) {
                    val columnName = cursor.getString(cursor.getColumnIndex("name"))
                    when (columnName) {
                        "cloudUserId" -> hasCloudUserId = true
                        "cloudEnabled" -> hasCloudEnabled = true
                        "subscriptionEnabled" -> hasSubscriptionEnabled = true
                    }
                }
                cursor.close()

                // Add cloud sync columns if they don't exist
                if (!hasCloudUserId) {
                    database.execSQL("ALTER TABLE `user` ADD COLUMN `cloudUserId` TEXT")
                }
                if (!hasCloudEnabled) {
                    database.execSQL("ALTER TABLE `user` ADD COLUMN `cloudEnabled` INTEGER NOT NULL DEFAULT 0")
                }
                if (!hasSubscriptionEnabled) {
                    database.execSQL("ALTER TABLE `user` ADD COLUMN `subscriptionEnabled` INTEGER NOT NULL DEFAULT 0")
                }
            }
        }
