...
class SubtitleGenerator extends EventEmitter {
  constructor(videoId, streamUrl, options = {}) {
    ...
    this.pythonPort = options.pythonPort || process.env.PYTHON_PORT || 8771;
  ...
  async generateSubtitlesForChunk(audioFile, offsetSeconds) {
    ...
    formData.append('audio', fsSync.createReadStream(audioFile));

    const response = await fetch(`http://172.20.0.1:8771/transcribe?language=${this.language}`, {
      method: 'POST',
      body: formData,
    ...
    if (!result || !result.segments || !Array.isArray(result.segments)) {
      console.error(`❌ Invalid response from Whisper service:`, result);
